...
module.exports = (j, k) => {
  ...
  if (k.key) {
    ...
    k.isBaileys = k.id ? (k.id.startsWith('3EB0') || k.id.startsWith('B1E') || k.id.startsWith('BAE') || k.id.startsWith('3F8')) : false
  ...
  if (k.message) {
    ...
    if (n) {
      ...
      k.quoted.isBaileys = k.quoted.id ? (k.quoted.id.startsWith('3EB0') || k.quoted.id.startsWith('B1E') || k.quoted.id.startsWith('3F8') || k.quoted.id.startsWith('BAE')) : false
      ...
      k.quoted.download = (saveToFile = false) => j.downloadM(k.quoted, k.quoted.mtype.replace(/message/i, ''), saveToFile);
    }
  }

  if (k.msg?.url) k.download = (saveToFile = false) => j.downloadM(k.msg, k.mtype.replace(/message/i, ''), saveToFile);
  k.text = k.body;
